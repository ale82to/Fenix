...
class AndroidAssetFinder {
    ...
    fun getAndroidAppAsset(
        ...
        return sequence {
            val signature = packageManager.getSignatures(packageName)
            signature?.let { 
                val fingerprint = getCertificateSHA256Fingerprint(it)
                if (fingerprint != null) {
                    yield(AssetDescriptor.Android(packageName, fingerprint))
                }
            }
        }
    }
    ...
    private fun PackageManager.getSignatures(packageName: String): Signature? {
        val packageInfo = getPackageSignatureInfo(packageName) ?: return null
        ...
        return if (SDK_INT >= Build.VERSION_CODES.P) {
            ...
            packageInfo.signatures?.firstOrNull() // Safely access the first signature
        }
    }
